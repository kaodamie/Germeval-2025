...
Your task is to decide whether the comment contains *candy speech* — defined as *cool*, positive, respectful, or encouraging language. think and reason  to come out with a viabel output for "yes" or "no"

Respond with only: **yes** or **no**. If uncertain, choose 'no' to reduce errors'''
...
df = pd.read_csv(r"test data\comments.csv")
# Apply async classifier
comments = df["comment"].tolist()
predictions = await batch_classify_comments(comments, batch_size=100)

# Attach predictions
df["task1_prediction"] = predictions

# Save predictions
df.to_csv(r"nemo_task1_predictions.csv" , index=False)
...
df.to_csv("C:\Users\kaoda\Desktop\PHD\Candy speech detection\osfstorage-archive\Data\test data\zero_shot_predictions_cleaned.csv", index=False)
